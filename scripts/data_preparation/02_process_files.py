...
# Assuming your pii_redaction script is named this and has the specified function
# Note: Based on your previous implementation, I'm assuming the actual function
# you want to call is `redact_pii` with a pattern list.
...
# ... (Boilerplate and Comment Marker constants also unchanged)
...
# NEW: Centralized helper function for alphanumeric ratio calculation
def _calculate_alnum_ratio(text: str) -> float:
    """Calculates the ratio of alphanumeric characters in a string."""
    if not text:
        return 0.0
    # Using a generator expression with sum() is highly efficient
...
def process_document(doc: Dict[str, str]) -> Dict:
    """
    The primary worker function for each parallel process.
    """
    ...
    # Note: Assuming your PII module has `redact_pii` and `ALL_PATTERNS_RECOMMENDED`
    # as per our earlier discussions for a final robust implementation.
    ...
    with ProcessPoolExecutor(max_workers=8) as executor, open(processed_data_path, 'w', encoding='utf-8') as f_out:
        ...
        for future in progress_bar:
            ...
            try:
                ...
                if status == "passed":
                    processed_doc = result["doc"] # This now contains the enriched document
